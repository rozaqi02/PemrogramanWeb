...
// Memeriksa apakah token CSRF ada dalam header permintaan
if(isset($headers['Csrf-Token'])) {
    // Jika ada maka dibandingkan dengan token CSRF yang disimpan di sesi
    if($headers['Csrf-Token'] !== $_SESSION['csrf_token']) {
        // Jika token tidak cocok maka mengeluarkan pesan kesalahan JSON
        exit(json_encode(['error' => 'Wrong CSRF Token']));
    }
} else {
    // Jika tidak ada token CSRF dalam header permintaan maka mengeluarkan pesan kesalahan JSON
